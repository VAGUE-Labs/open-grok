    ...
    local
        .run_until(async {
            ...
            let actor = SessionActor {
                ...
                git_head_enabled: false,
                models_manager: Default::default(),
                cache_tracker: std::cell::RefCell::new(
                    crate::session::cache_tracker::CacheTracker::new(),
                ),
                display_cwd: std::sync::OnceLock::new(),
                active_agent_type: parking_lot::Mutex::new(None),
            ...
            // The production entrypoint refuses the loopback mock (only the
            // cli-chat-proxy host is trusted), so it must be a no-op here.
            actor.maybe_refresh_model_metadata_on_resume().await;
            let cfg_after_production = actor.chat_state_handle.get_sampling_config().await.unwrap();
            assert_eq!(
                cfg_after_production.context_window,
                std::num::NonZeroU64::new(200_000).unwrap(),
                "the production entrypoint must reject the loopback mock"
            );
            // The test-only loopback seam exercises the real HTTP fetch/parse.
            actor
