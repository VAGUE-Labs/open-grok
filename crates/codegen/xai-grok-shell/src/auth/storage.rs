...
/// Read the Wafer AI API key from its isolated auth.json scope.
pub fn read_wafer_api_key(grok_home: &Path) -> Option<String> {
    let path = grok_home.join("auth.json");
    let map = read_auth_json(&path).ok()?;
    map.get(WAFER_API_KEY_SCOPE)
        .map(|auth| auth.key.clone())
        .filter(|key| !key.trim().is_empty())
}

/// Return whether a non-empty Wafer AI credential exists without cloning it.
pub fn wafer_api_key_is_configured(grok_home: &Path) -> bool {
    let path = grok_home.join("auth.json");
    let Ok(map) = read_auth_json(&path) else {
        return false;
    };
...
/// Store a Wafer AI API key in the provider-isolated auth.json scope.
pub fn store_wafer_api_key(grok_home: &Path, api_key: &str) -> std::io::Result<()> {
    let api_key = api_key.trim();
    if api_key.is_empty() {
        return clear_wafer_api_key(grok_home);
    }
    store_scoped_api_key(grok_home, WAFER_API_KEY_SCOPE, api_key)
}

/// Clear only the Wafer AI API-key scope.
pub fn clear_wafer_api_key(grok_home: &Path) -> std::io::Result<()> {
...
mod provider_api_key_tests {
    ...
    #[test]
    fn wafer_key_round_trip_and_clear_preserve_unrelated_scopes() {
        let dir = tempfile::tempdir().unwrap();
        store_api_key(dir.path(), "xai-secret").unwrap();
        store_perplexity_api_key(dir.path(), "pplx-secret").unwrap();
        ...
        assert!(wafer_api_key_is_configured(dir.path()));

        clear_wafer_api_key(dir.path()).unwrap();
    ...
    #[test]
    fn empty_wafer_key_clears_only_its_scope() {
        let dir = tempfile::tempdir().unwrap();
        store_wafer_api_key(dir.path(), "wafer-secret").unwrap();
        store_api_key(dir.path(), "xai-secret").unwrap();
