...
mod tests {
    ...
    #[test]
    fn plugin_alias_keeps_the_builtin_and_honors_visibility() {
        let mut registry = CommandRegistry::new(builtin_commands());
        registry.set_acp_commands(&[acp::AvailableCommand::new(
            "plugin".to_string(),
            "Shell plugin command".to_string(),
        )]);
        assert_eq!(registry.get("plugin").unwrap().name(), "plugins");
        registry.set_plugins_visible(false);
    ...
    fn shell_collision_contract_covers_every_pager_command_and_alias() {
        let reserved = xai_grok_shell::session::PAGER_COMMAND_KEYS;
        for command in builtin_commands() {
            for key in std::iter::once(command.name()).chain(command.aliases().iter().copied()) {
                assert!(reserved.contains(&key), "unreserved pager key {key}");
            }
        }
    }
    #[test]
