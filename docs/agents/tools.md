...
- `workflow` launches a **Rhai** orchestration script through the native workflow runtime: the pure engine lives in `crates/codegen/xai-workflow/` (interpreter, journal, meta/validate) and the session binding in `xai-grok-shell/src/session/workflow/` (`manager`, `host_service`, `tracker`, `store`, `registry`, `notify`, `schema_contract`). The script must begin with `let meta = #{ name, description, when_to_use?, phases? }` as a literal; the body drives `agent()`, `parallel()`, `phase()`, `log()`, `complete()`, `pause()`, `await_user()`, `escalate()`, `budget()`, `render_template()`, scratch-file helpers, `git_diff_since()`, `fingerprint()`, and `json_encode()`.
- The tool takes exactly one source — `name` (registry), inline `script`, or `script_path` — plus optional `args`, `agent_budget` (default 128, max 1024 agent calls), `resume_from_run_id`, `resume_note` (only with `resume_from_run_id`, only while the run is blocked at `escalate()`; the gate returns it to the script), and `validate_only`. Launch is decoupled: the tool sends a `WorkflowLaunchRequest` over an mpsc handle; the session resolves the source and `WorkflowManager::launch()` runs the interpreter on `spawn_blocking`, capped at 4 active runs per session.
- Every `agent()` call is serviced by `host_service.rs`: it builds a `SubagentRequest` (model/reasoning_effort/agent_type/capability_mode/worktree isolation from the opts map, `owner = SubagentOwner::workflow(run_id)`) and sends `SubagentEvent::Spawn` down the same coordinator path as `task`/`agent_swarm` — so model and effort overrides resolve identically. `output_schema` compiles a JSON-Schema contract with fenced-JSON extraction and one corrective retry.
...
- Lifecycle + UI: runs are always background; state changes broadcast as `WorkflowUpdated` ext-notifications that feed the pager's inline workflow scrollback block and the `/workflows` overlay (drill-down, pause/resume/stop/save). Terminal states AND script-initiated pauses (`blocked`/`infra`/`back_off`/`no_progress` — everything except a deliberate user pause) inject a `WorkflowCompletionTurn` back into the model's context — no polling; the reminder carries the blocking issue plus resume instructions, so the model fixes the cause and resumes instead of the run just stopping. `escalate(message)` is the script-facing gate for this: a journaled one-shot pause whose resume returns the tool-supplied `resume_note` to the script (a failed run's trailing host error is pruned on resume so the failed step re-executes live). `/workflow pause|resume|stop|save <name>` manages runs by display name.
- Registry: builtin (`deep-research` and `ultracode`, `include_str!`'d) > project (`<repo>/.opengrok/workflows/*.rhai`, folder-trust + `O_NOFOLLOW` gated) > user (`~/.opengrok/workflows/*.rhai`); registered workflows also surface as their own slash commands. `ultracode` is the maximum-rigor coding orchestrator (scout + parallel readers → judged designs → implementer → adversarial review lenses with bounded fix rounds) and escalates unresolvable blockers back to the session model. Depth rules unchanged: workflows launch at depth 0 only, and child toolsets strip `task`/`agent_swarm`/`workflow` at depth 1.
